...
In some rare cases, HDB would perform worst than HIFB. Figure~\ref{fig:imbalance_hifb_hdb} shows a case when merging $t_1, t_2, t_3, t_4, t_5$, HDB does not tell the difference between them since the distances between each pair of them are all 2. However, HIFB does distinguish them since their impact factors are quite different. This example exists in some of the LIGO workflow which we will discuss its details in Section.~\ref{sec:results}. 

\begin{figure}[htb]
	\centering
	\includegraphics[width=0.85\linewidth]{figures/imbalance/hifb_vs_hdb.pdf}
	\captionof{figure}{An example when HDB does not distinguish tasks while HIFB does}
