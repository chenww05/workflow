...
Figure~\ref{fig:model_vc} illustrates an example of vertical clustering, in which the pipeline containing tasks $t_2$, $t_4$, and $t_6$ is merged into $j_1$, while the pipeline containing tasks $t_3$, $t_5$, and $t_7$ is merged into $j_2$. Similarly, clustering delays $c_2$ and $c_3$ are added to $j_1$ and $j_2$ respectively, but system overheads $s_4$, $s_5$, $s_6$, and $s_7$ are removed. 
